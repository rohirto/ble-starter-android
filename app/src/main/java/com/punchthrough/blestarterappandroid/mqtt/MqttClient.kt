...
const val MQTT_HOST = "tcp://demo.thingsboard.io:1883"
...
const val MQTT_ACCESS_TOKEN_1 = "uN1kzFzsaBH94zFctiNn"
...
class MqttClientHelperTag(context: Context?)  {
    companion object {
        const val TAG = "MqttClientHelperTag"
    }
...
class MqttClientHelperGPS(context: Context?)  {
    companion object {
        const val TAG = "MqttClientHelperGPS"
    }
    ...
    private fun connect() {
        ...
        mqttConnectOptions.connectionTimeout = MQTT_CONNECTION_TIMEOUT
        mqttConnectOptions.keepAliveInterval = MQTT_CONNECTION_KEEP_ALIVE_INTERVAL
        mqttConnectOptions.userName = MQTT_ACCESS_TOKEN_1
        try {
            mqttAndroidClient.connect(mqttConnectOptions, null, object : IMqttActionListener {
                override fun onSuccess(asyncActionToken: IMqttToken) {
                    val disconnectedBufferOptions =
